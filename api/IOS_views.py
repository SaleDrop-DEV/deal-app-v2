...
def IOS_API_fetch_my_sales_old(request):
    sleep(SLEEP_TIME)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def IOS_API_fetch_my_feed_old(request):
    def parse_date_received(date_received):
        now = timezone.now()
        delta = now - date_received
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            gender_preference_user = ["M", "F", "B"][request.user.extrauserinformation.gender]
            acceptable_genders = ['B']
            #new stores can only go back at most 4 days
            four_days_ago = timezone.now() - timedelta(days=4)

            if gender_preference_user != "B":
                acceptable_genders.append(gender_preference_user)
                new_stores = list(deals_models.Store.objects.filter(gender__in=acceptable_genders, dateIssued__gte=four_days_ago).all())
            else:
                new_stores = list(deals_models.Store.objects.filter(dateIssued__gte=four_days_ago).all())

            feed_items = sales + new_stores
            feed_items.sort(key=lambda item: item.message.received_date if hasattr(item, 'message') else item.dateIssued, reverse=True)
